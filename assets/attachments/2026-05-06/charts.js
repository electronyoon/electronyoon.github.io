...
const crossCtx = document.getElementById('crossContaminationChart');
if (crossCtx) {
  new Chart(crossCtx, {
    type: 'bar',
    data: {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'QWGJK 제외 API — 응답 지연 비율 (30일)',
        },
        subtitle: {
          display: true,
          text: '자체 max가 472ms인 API들이 왜 500ms를 넘었나?',
...
const hourlyCtx = document.getElementById('hourlyExcludedChart');
if (hourlyCtx) {
  ...
  new Chart(hourlyCtx, {
    ...
    options: {
      ...
      plugins: {
        ...
        subtitle: {
          display: true,
          text: '3월 새벽 1~2시: 자체 max(472ms)를 넘는 지연 → cascade 오염의 증거',
...
const qwgOnlyCtx = document.getElementById('hourlyQwgjkOnlyChart');
if (qwgOnlyCtx) {
  const hours = Array.from({ length: 24 }, (_, i) => `${i}시`);
...
const allHourlyCtx = document.getElementById('hourlyAllChart');
if (allHourlyCtx) {
  const hours = Array.from({ length: 24 }, (_, i) => `${i}시`);
...
const pctQwgjkOnlyCtx = document.getElementById('percentileQwgjkOnlyChart');
if (pctQwgjkOnlyCtx) {
...
const pctExcluded30Ctx = document.getElementById('percentileExcluded30Chart');
if (pctExcluded30Ctx) {
...
// Chart 4: 전체 엔드포인트 백분위수 (30일, P1~P99 라인)
const pctAll30Ctx = document.getElementById('percentileAll30Chart');
if (pctAll30Ctx) {
  const pctLabels = Array.from({ length: 99 }, (_, i) => `P${i + 1}`);
...
const slowQwgjkCtx = document.getElementById('slowRatioQwgjkChart');
if (slowQwgjkCtx) {
  new Chart(slowQwgjkCtx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'QWGJK 응답 지연 비율 (30일)',
        },
        subtitle: {
          display: true,
          text: '트래픽 2.75배 증가에도 모든 구간 개선',
          font: { size: 12, style: 'italic' },
          padding: { bottom: 10 },
        },
        tooltip: {
