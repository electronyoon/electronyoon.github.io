...
*최초 작성일: 2024.11.05 최종 수정일: 2024.11.25*
...
## Table of contents
1. [소개](#소개)
2. [Java 바이트 코드는 어떻게 생겼을까](#java-바이트-코드는-어떻게-생겼을까)
3. [바이트 코드를 사용하는 이유](#바이트-코드를-사용하는-이유)
4. [중간 언어의 역사와 추상 기계](#중간-언어의-역사와-추상-기계)
5. [TLDR](#TLDR)
...
- 중간 언어란 원시*source* 언어를 목적*target* 언어로 변환하는 작업 중간에 생성되는 언어입니다.[^1]
...
```
파일이 다 그렇듯 맨 앞은 메타데이터를 포함합니다. *cafe babe*는 자바 클래스 파일의 매직넘버입니다. 이에 관한 [재밌는 일화](https://velog.io/@hyeonwoody/CAFㅌ₩EDEAD-CAFEBABE-사라지고-살아있는-매직넘버)가 있습니다.
...
<img src="{{ site.baseurl }}/assets/attachments/2024-11-05-01/Figure 1.png" alt="Figure 1" style="width: 50%; height: auto;" />
...
IL(중간 언어)은 컴파일 경우의 수를 s\*t에서 s+t로 줄여 *"portable"*하게 만든다는 것이 핵심입니다.

Java의 창시자 James Gosling 또한 동일한 취지의 언급을 남겼습니다. 다음은 Oak에 관한 발표 중 일부입니다[^5]. (Oak은 프로그래밍 언어로, Java의 전신입니다)
...
와 같이 많은 프로그래밍 언어들이 이미 *"virtual machine"*을 사용하고 있었으며, 그 시초는 UNCOL이라는 개념이라고 합니다. 위키피디아에서 [UNCOL](https://en.wikipedia.org/wiki/UNCOL#cite_ref-2)을 찾아보면:
...
> "...some authors use the terms `emulator` or `interpreter` and some use the term `virtual machine` for implementations of abstract machines, similar as we use the term `program` for implementations of an algorithm."
...
꽤 먼 길을 다녀 왔습니다. 정리해 보겠습니다.

- 바이트 코드는 중간 언어의 일종입니다. JVM뿐만 아니라 다른 언어에서도 폭넓게 사용됩니다. 즉, 바이트 코드는 반드시 자바 바이트 코드만을 의미하지는 않습니다.
- 중간 언어란 컴파일러에서 사용하는 추상 계층의 언어입니다. 중간 언어의 장점은 많지만, 특히 플랫폼 이식성을 높여준다는 점이 높게 평가됩니다.
