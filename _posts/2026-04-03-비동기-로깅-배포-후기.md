...
**응답시간은 전체적으로 개선되었지만, 극단적인 꼬리 지연은 여전합니다. 이 문제는 페이지네이션 쿼리 문제로 확인됐습니다.**
...
100배 정도 차이납니다. *(심지어 전국구 자료 조회는 인덱스까지 활용합니다![^1])*

문제는 우리 API가 모두 정렬과 페이지네이션을 사용하기 때문에, 서브 쿼리의 크기가 클수록 비효율적인 access-filter 과정을 거친다는 점입니다.[^2]
...
- 페이지네이션을 keyset으로 변경하거나
- 인덱스를 더 잘 활용하도록 수정/추가

하는 것입니다. 또, 현재 결과는 배포 전후 2일치에만 국한되므로
...
[^1]: 이 API은 한 개의 테이블에 대응하며, 테이블에는 인덱스가 두 개 있습니다.
