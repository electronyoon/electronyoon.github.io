...
[^5]: <p>테스트는 최대 ➊150만개의 레코드 테이블에 ➋200번의 요청을 10번 보내는 것으로 진행합니다.</p>➊의 값은 2023년 기준 대한민국의 [총 건물 수](https://stat.molit.go.kr/portal/cate/statView.do?hRsId=19&hFormId=540&hSelectId=540&hPoint=00&hAppr=1&hDivEng=&oFileName=&rFileName=&midpath=&sFormId=540&sStart=2023&sEnd=2023&sStyleNum=94&settingRadio=xlsx)에서 추정했습니다.<br>➋의 값은 여러번의 실험을 통해 추정했습니다. 실험이 실제 세계를 대표하려면 샘플의 수가 충분히 확보되어야 합니다. 이 실험에서 샘플 수는 곧 총 요청 수와 같습니다. 백분위수 추정에 필요한 샘플 수를 구하는 것은 깊은 통계학적 지식을 요구해 대략적인 필요치만을 조사했습니다(15,000~17,000).<span class="math">표본\ 수\ n=50 \times x \times y \times z</span>50은 RPS 값, x는 부하 시간, y는 독립 시행 횟수, z는 부트스트랩 횟수입니다. x를 곱하는 것은 부하 테스트를 뜻하고, y를 곱하는 것은 독립적인 실험을 여러 번 수행함을 뜻합니다. z를 곱하는 것은 이미 추출된 표본에서 재추출을 통해 표본 수를 늘리는 것을 뜻합니다. 여기서 (x, y, z)를 조정하며 실험 결과를 관찰했고, 최종 값은 (4, 10, 1000)으로 정했습니다.

[^6]: <p>피크타임에서 사용자 95%의 지연 시간을 재현하기 위해 P95값을 추정했습니다. 계산 결과 ➊PostGIS는 418.9ms, ➋일반 쿼리는 2148.7ms입니다.</p><p>이는 피크타임에서 ➊PostGIS 사용자들의 95%는 418.9ms보다 낮은 지연시간을, ➊일반 쿼리 사용자들의 95%는 2148.7ms보다 낮은 지연시간을 경험할 것이라는 뜻입니다.</p>보다 자세한 결과는 [다음](https://rawcdn.githack.com/f-lab-edu/zipsoon/193b8b6edbbca38537380d0767e19f983668dbe1/api/postgis_performance_summary_rep10.html)을 참조하세요. 샘플 수가 더 많은 실험은 [다음](https://rawcdn.githack.com/f-lab-edu/zipsoon/193b8b6edbbca38537380d0767e19f983668dbe1/api/postgis_performance_summary_old.html)을 참조하세요.
