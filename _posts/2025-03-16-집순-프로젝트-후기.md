...
- 그러므로 먼저 집순 서비스가 견뎌야 할 `부하의 상방`을 추정해 보겠습니다.
- 초당 요청 수(Requests Per Second, RPS)은 다음 공식으로 계산합니다.
...
- 한 유저가 ***평균 20-30회정도를 요청***한다고 가정합니다.


### iii) Peak, Time: 0.1, 7200초

- 프롭테크 서비스의 시간-요일-계절별 트래픽 패턴은 알려진 바가 없습니다.
- 다만 이커머스의 경우, 시간대별 구매 패턴이 잘 알려져 있습니다.[^3] 이에 따르면 10시와 12시에 가장 많은 구매가 일어납니다.[^4]
