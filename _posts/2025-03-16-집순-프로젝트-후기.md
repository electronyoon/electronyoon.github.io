...
- 후술할 2개의 챌린지는 "요청을 얼마나 효율적으로 처리하는지"와 관련이 있습니다.

- 그러므로 집순 프로젝트의 이용자 수와 트래픽을 간단히 추정해, 앱이 어디까지 견뎌야 할지 '상방'을 가정해 보겠습니다.

- 피크 트래픽(초당 요청 수, RPS)은 다음 공식으로 계산합니다:
...
- 집순은 신생 서비스이므로, 직방의 약 1/3인 `10만 DAU를 가진다고 가정`하겠습니다.
...
### 피크 시간대 비율과 길이: 10%, 7200초

- 프롭테크 서비스의 시간-요일-계절별 트래픽 패턴은 잘 알려진 바가 없습니다.

- 다만 이커머스의 경우, 시간대별 구매 패턴이 알려져 있습니다.[^3]

- 위 조사에 따르면 10시와 12시에 가장 많은 구매가 일어나며, 각각 약 6.7%를 차지합니다.[^4]

- 이에 따라 `피크 시간대 비율은 10%`로, `피크 초는 7,200초(2시간)`으로 가정하겠습니다. 간편한 계산을 위해 구매 트래픽을 조회 트래픽으로 단순 변환했습니다.

### 결론

<span class="math">RPS = \frac{100,000 \times 30 \times 0.1}{7,200} = \frac{300,000}{7,200} = 41.67</span>

- 따라서 집순은 **최대 초당 약 42개의 요청**을 처리할 수 있어야 합니다.
...
[^4]: <table>
        ...
        <tbody>
            ...
            <tr><td>10</td><td>56</td><td>6.67</td></tr>
            <tr><td>11</td><td>53</td><td>6.32</td></tr>
            <tr><td>12</td><td>56</td><td>6.67</td></tr>
